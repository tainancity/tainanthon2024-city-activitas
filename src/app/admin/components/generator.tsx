...
function AssetStatusPieChart({ title, icon, inputData, colors }) {
    ...
    const statusCounts = inputData.reduce((acc, item) => {
...
function IdleAssetStackedBarChart({ title, icon, inputData }) {
    const processData = () => {
        const assetTypes = [...new Set(Object.values(inputData).map(item => item.資產類型))];
        const districts = [...new Set(Object.values(inputData).map(item => item.行政區))];
        
        const data = assetTypes.map(type => {
            return {
            y: districts,
            x: districts.map(district => 
                Object.values(inputData).filter(item => item.行政區 === district && item.資產類型 === type).length
            ),
            name: type,
            orientation: 'h',
            type: 'bar',
            marker: { color: pastelColors[assetTypes.indexOf(type) % pastelColors.length] },
            };
        });
...
function CaseStackedBarChart({ title, icon, inputData }) {
    const processData = () => {
        const assetTypes = [...new Set(Object.values(inputData).map(item => item.任務狀態))];
        const agencies = [...new Set(Object.values(inputData).map(item => item.執行機關))];
        
        const data = assetTypes.map(type => {
            return {
            y: agencies,
            x: agencies.map(agencie => 
                Object.values(inputData).filter(item => item.執行機關 === agencie && item.任務狀態 === type).length
            ),
            name: type,
            orientation: 'h',
            type: 'bar',
...
const AssetMap: React.FC<AssetMapProps> = ({ title, idleAssetData }) => {
    const processData = () => {
      ...
      data.forEach(asset => {
        ...
        latitudes.push(lat);
        longitudes.push(lng);
        texts.push(`${asset.標的名稱}<br>類型: ${asset.資產類型}<br>地址: ${asset.地址}`);
